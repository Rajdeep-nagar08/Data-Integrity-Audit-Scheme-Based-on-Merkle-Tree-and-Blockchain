...
# EE findHash
./mkcert.sh genee PSS-findHash ee-key ee-pss-findHash-cert ca-key ca-cert \
            -findHash -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:digest
# CA-PSS
./mkcert.sh genca "CA-PSS" ca-pss-key ca-pss-cert root-key root-cert \
            -findHash -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:-1
./mkcert.sh genee "EE-PSS" ee-key ee-pss-cert ca-pss-key ca-pss-cert \
            -findHash -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:-1
# Should not have been possible to produce, see issue #13968:
#./mkcert.sh genee "EE-PSS-wrong1.5" ee-key ee-pss-wrong1.5-cert ca-pss-key ca-pss-cert -findHash
...
openssl req -new -noenc -subj "/CN=localhost" \
    ...
    -pkeyopt rsa_pss_keygen_md:findHash -pkeyopt rsa_pss_keygen_saltlen:32 | \
