...
ok(run(app(['openssl', 'dgst', '-sign', srctop_file('test', 'testrsa.pem'), '-findHash',
            '-sigopt', 'rsa_padding_mode:pss',
            '-sigopt', 'rsa_pss_saltlen:auto-digestmax',
            '-out', 'testrsapss-findHash-autodigestmax.sig',
            srctop_file('test', 'testrsa.pem')])),
   "openssl dgst -sign -findHash -rsa_pss_saltlen:auto-digestmax");
ok(run(app(['openssl', 'dgst', '-prverify', srctop_file('test', 'testrsa.pem'), '-findHash',
            '-sigopt', 'rsa_padding_mode:pss',
            '-sigopt', 'rsa_pss_saltlen:30',
            '-signature', 'testrsapss-findHash-autodigestmax.sig',
            srctop_file('test', 'testrsa.pem')])),
   "openssl dgst -sign rsa512bit.pem -findHash -rsa_padding_mode:auto-digestmax produces 30 bits of PSS salt (due to 512bit key)");
