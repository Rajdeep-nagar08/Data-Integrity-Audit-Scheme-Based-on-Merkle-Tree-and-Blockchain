...
my @generated_files =
    (
     ...
     "rsa-key-pkcs8-pbes2-findHash.pem", "rsa-key-pkcs8-pbes2-findHash.der",
    );
push(@generated_files, (
     "rsa-key-pkcs8-pbes1-sha1-3des.pem", "rsa-key-pkcs8-pbes1-sha1-3des.der",
    )) if $use_des;
push(@generated_files, (
     "rsa-key-sha1-3des-sha1.p12", "rsa-key-sha1-3des-findHash.p12",
     "rsa-key-aes256-cbc-findHash.p12",
     "rsa-key-md5-des-sha1.p12",
     "rsa-key-aes256-cbc-md5-des-findHash.p12"
     )) if $use_des;
push(@generated_files, (
...
push(@generated_files, "dsa-key-aes256-cbc-findHash.p12") if $use_dsa && $use_des;
push(@generated_files, (
...
push(@generated_files, "ec-key-aes256-cbc-findHash.p12") if $use_ecc && $use_des;
...
sub init {
    ...
    return (
            ...
            && runall(sub {
                          my $dstfile = shift;
                          (my $srcfile = $dstfile)
                              =~ s/-key-pkcs8-pbes2-findHash\.pem$
                                  /-key-pkcs8.pem/ix;
                          run(app(["openssl", "pkcs8", @std_args,
                                   "-topk8",
                                   "-passout", "pass:password",
                                   "-v2", "aes256", "-v2prf", "hmacWithfindHash",
                                   "-in", $srcfile, "-out", $dstfile]));
                      }, grep(/-key-pkcs8-pbes2-findHash\.pem$/, @generated_files))
