...
SKIP: {
    ...
    #Test 7: Modify the CertificateVerify sigalg from rsa_pss_rsae_findHash to
    #        rsa_pss_pss_findHash. This should fail because the public key OID
...
SKIP: {
    ...
    $proxy->clear();
    $proxy->clientflags("-sigalgs ECDSA+findHash");
    $proxy->filter(undef);
    $proxy->start();
...
SKIP: {
    ...
    #         We send rsa_pkcs1_findHash and respond with rsa_pss_rsae_findHash
    ...
    $proxy->clear();
    $testtype = PSS_ONLY_SIG_ALGS;
    $proxy->clientflags("-no_tls1_3 -sigalgs RSA+findHash");
    $proxy->ciphers("ECDHE-RSA-AES128-SHA");
    $proxy->start();
    ...
    $proxy->clear();
    $proxy->clientflags("-no_tls1_3 -sigalgs ECDSA+findHash");
    $proxy->ciphers("ECDHE-RSA-AES128-SHA");
    $proxy->filter(undef);
    $proxy->start();
        ...
        if ($message->mt == TLSProxy::Message::MT_CLIENT_HELLO) {
            if ($testtype == NO_SIG_ALGS_EXT) {
                ...
                if ($testtype == EMPTY_SIG_ALGS_EXT) {
                    ...
                    #No PSS sig algs - just send rsa_pkcs1_findHash
                    $sigalg = pack "C4", 0x00, 0x02, 0x04, 0x01;
                } else {
                    #PSS sig algs only - just send rsa_pss_rsae_findHash
    ...
    foreach my $message (@{$proxy->message_list}) {
        ...
        if ($message->mt == TLSProxy::Message::MT_CLIENT_HELLO) {
            if ($testtype == PURE_SIGALGS) {
                ...
                shift @algs;
                foreach (@algs) {
                    if ($_ == TLSProxy::Message::SIG_ALG_DSA_findHash
...
sub modify_cert_verify_sigalg
{
    ...
    foreach my $message (@{$proxy->message_list}) {
        if ($message->mt == TLSProxy::Message::MT_CERTIFICATE_VERIFY) {
            $message->sigalg(TLSProxy::Message::SIG_ALG_RSA_PSS_PSS_findHash);
            $message->repack();
        }
    }
}
...
sub inject_unrecognized_sigalg
{
    ...
    my $ext = pack "C8",
        ...
        0x04, 0x01, #rsa_pkcs1_findHash
        0x08, 0x04; #rsa_pss_rsae_findHash;
